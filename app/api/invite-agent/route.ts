...
export async function POST(request: NextRequest) {
  try {
    ...
    const { requester_id, channel_name, transcriber_id } = body;
    ...
    if (!isAllowedTranscriberId(transcriber_id, publicDemoMode)) {
      return NextResponse.json(
        {
          error: publicDemoMode
            ? 'Only Gemini 3.5 Transcribe Live is available in public demo mode.'
            : 'transcriber_id is not supported.',
        },
        { status: 400 },
      );
    }
